...
func main() {
	...
	// Admin Routes - register under both /admin and /api/admin prefixes for maximum proxy compatibility
	adminGroups := []*gin.RouterGroup{
		adminApp.Group("/admin", jwtMid),
		adminApp.Group("/api/admin", jwtMid),
	}
	...
	for _, admin := range adminGroups {
		...
		iam.PUT("/users/:username/password", adminHandler.UpdatePassword)
